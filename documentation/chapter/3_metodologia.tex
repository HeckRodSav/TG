...
\begin{itemize}
	...
	\item \lstinline|d|, distância \ac{d} entre par de antenas do arranjo;
	\item \lstinline|D_signals|, ?? ;
	...
	\item \lstinline|N_antenas|, quantidade \ac{Nant} de antenas do arranjo;
	\item \lstinline|omega_w|, frequência angular \ac{omega};
	\item \lstinline|phase_w|, fase $\phi$ do sinal no emissor;
	\item \lstinline|Rho|, raio \ac{rho} do polígono que dispõe as antenas no arranjo;
...
\begin{itemize}
	\item \lstinline|AoA_range|, ?? ;
	\item \lstinline|ant_array|, coordenadas das antenas do arranjo;
	\item \lstinline|delta_A_x_B_array|, contendo o ângulo \ac{thetak} calculado por $\ac{alphak} + \ac{betak}$ aferido para cada par de antenas do arranjo;
	\item \lstinline|delta_B_x_A_array|, contendo o ângulo \ac{thetak} calculado por $\ac{alphak} - \ac{betak}$ aferido para cada par de antenas do arranjo;
	\item \lstinline|Z_phase_array|, matriz de valores numéricos complexos, contendo o sinal complexo aferido para cada antena do arranjo;
	\item \lstinline|Z_phase_matrix|, ?? ;
	\item \lstinline|z_plot|, estado corrente do sinal no espaço, utilizado na geração de imagem da simulação;
	\item \lstinline|Z_x_array|, valores complexos, contendo a defasagem \ac{DeltaPhi} aferido para cada par de antenas no arranjo;
\end{itemize}

Parâmetros de outros tipos:
\begin{itemize}
	\item \lstinline|steering_function|, ?? ;
	\item \lstinline|steering_arguments|, ?? ;
\end{itemize}
...
% calc_AoA_polygon
A primeira grande função desenvolvida foi \lstinline|calc_AoA_geometric|, que é responsável pelo cálculo geral da simulação.
...
O \autoref{cod:calc_AoA_geometric} apresenta uma versão simplificada da função \lstinline|calc_AoA_geometric| desenvolvida.
...
\begin{lstfloat}[htbp]
	...
	\lstinputlisting[
			basicstyle=\ttfamily\small\setstretch{1},
			label=cod:calc_AoA_geometric,
			caption={Função \lstinline|calc_AoA_geometric|, simplificada.}
		]{../code/calc_AoA_geometric_alt.m}
	\caption*{Fonte: Autor.}
\end{lstfloat}
...
\begin{figure}[htbp]
    \centering
    \caption{Fluxograma de operações da função \lstinline|calc_AoA_geometric|.}
    \input{../pictures/fluxograma}
...
Os valores exibidos são calculados pela função \lstinline|calc_AoA_geometric|.
...
Finalmente a função responsável por juntar todas as partes é \lstinline|w_xyt|, a base para a simulação, ela invoca as funções \lstinline|calc_AoA_geometric| e \lstinline|generate_fig| com os devidos parâmetros, além de garantir que os arquivos gerados sejam salvos corretamente.
...
% Essa função também invoca a simulação do algoritmo de \ac{GN} para as mesmas condições.

% \section{Simulação do Algoritmo de \acs{GN}}

% Para análise comparatória, também foi desenvolvida uma simulação para o método de \ac{AoA} utilizando o algoritmo de \ac{GN}, adaptando a proposta de \citeauthor{Horst2025BTLEAoA} \cite{Horst2025BTLEAoA}, referida na \autoref{sec:trabalhos_relacionados}.
